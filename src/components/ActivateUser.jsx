...
export const ActivateUser = ({ dbUsers }) => {
  ...
  const activeUsers = dbUsers.filter((user) => user.active);
  const notActiveUsers = dbUsers.filter((user) => !user.active);
  ...
  const handleSubmitDesactivate = (e) => {
    e.preventDefault();
